...
python3 tools/selftest-decode.py   # ~95 assertions, base can be powered off
...
`pedal-web.py` is the product; the other three modules are libraries it imports.
...
- **`sysstate.py`** — answers "is this machine in a state where the page can be
  ...
  fixes it and never runs it. Keep it that way.
- **`ffb.py`** — the bounded FFB test. Measures torque by reading `ABS_X` back
  off the same `O_RDWR` fd that uploaded the effect, through evdev rather than
  the dashboard's own STEER channel (the event node exists whenever the driver is
  bound; hidraw points at the real base only under `hidraw_pid=0`).
- **`pedal-web.py`** — a reader thread at full report rate, a latched snapshot,
  and a `BaseHTTPRequestHandler` serving `GET /`, `GET /data`, `GET /system`,
  `POST /reset`, `POST /ffb/{start,abort}`.

### The report map is duplicated in five places

Change the byte offsets and five sites must move together, or the tool becomes
confidently wrong — the worst failure mode here:

1. `hid_layout.KNOWN_OFFSETS` / `KNOWN_SIZE` — the sanity check on the parse
2. `hid_layout.fallback_layout()` — used whenever the descriptor is unreadable
3. `pedal-web.decode_vendor()` — hardcodes `len(rep) != 33`
4. `docs/report-map.md` — `build_layout()` literally emits *"docs/report-map.md
   offsets are stale"* when the parsed descriptor disagrees
5. Sections [1] and [2] of the self-test

### Invariants that are easy to break by accident

- **Latch, don't sample.** `SEEN` (min/max ever) is separate storage from `HIST`
  (rolling window) so a press that ages out of the sparkline is still latched.
  The dashboard exists to catch faults nobody was watching for.
...
- **Locks stay independent.** `sysstate._LOCK` and `ffb._LOCK` exist so neither
  ever waits on `pedal-web.LOCK` at the 10 Hz poll rate. Don't cross them.
...
- `pedal-web.py` is one 1191-line file, ~570 of it the inline `PAGE` string with
  its own CSS and JS. The zero-dependency single file is why it runs anywhere
  Python 3 does.
